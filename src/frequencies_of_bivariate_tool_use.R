...
survey_dir <- "../datafiles/survey"

survey_path_raw <- file.path(survey_dir, "survey_raw_variable_list.csv")
survey_path_cleaned <- file.path(survey_dir, "survey_cleaned.csv")

survey_raw_variable_list <- readr::read_csv(survey_path_raw) %>% 
  janitor::clean_names()

survey_cleaned <- readr::read_csv(survey_path_cleaned) # I love the 
